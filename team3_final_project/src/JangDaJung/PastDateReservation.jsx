...
function PastDateReservation() {
  ...
  const [userIdxList, setUserIdxList] = useState([])
  ...
  const token = localStorage.getItem('jwtTokenSjh');
  const decoded = decodeJwt(token);
  const userIdx = decoded.userIdx
  console.log(userIdx)

  useEffect(() => {
    axios.get("http://localhost:8080/pre/pastDateRes", {
      params: {
        userIdx: userIdx
      }, headers: {
        Authorization: `Bearer ${localStorage.getItem("jwtTokenSjh")}`
      }
    }).then((res) => {
      console.log(res.data)
      setPastReservationList(res.data)

      // res data 의 userIdx 들을 다시 userIdx 에 넣는다
      const extractedUserIdxList = res.data.map(item => item.userIdx)
      setUserIdxList(extractedUserIdxList)
    }).catch((err) => {
      console.log(err)
    })
  }, [])
  ...
  useEffect(() => {
    console.log("-------userIdxList")
    console.log(userIdxList)

    if (userIdxList.length === 0) return;

    userIdxList.forEach((uid) => {
      axios.get("http://localhost:8080/pre/getUserInfo", {params: {userIdx: uid}})
    })
  }, [userIdxList]);
  ...
  // 검색 조건 필터링
  const filteredData = pastReservationList.filter(item => {
    const matchesSearch =
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td>
                        <p className={"mb-0"}>{item.rsvDate}</p>
                        <p className={"mb-0"}>{item.rsvTime}</p>
                      </td>
                      <td>{item.name}</td>
